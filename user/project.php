  ...
  $(function() {
    ...
    $( "#anim" ).change(function() {
      $( "#datepicker" ).datepicker( "option", "showAnim", $( this ).val() );
		...
		function add(user,fname,lname,reg_no,p_id){
		...
		function update(user,fname,lname,reg_no,p_id,marks){
				...
				<div class='middle_right'>
					<div class='middle' style='width:100%'>
						<div class='title_middle' style='margin-bottom:0pt;'><h2>Project</h2><?php echo $message ?></div>
						       		...
						       		if($row_course = mysqli_fetch_assoc($result_course))
						       		{
							        	...
							        	$result_class = $database->query($Query_class);
							       		...
							       		{ ?>
								        	...
								        	<b>Best Project :</b> <?php if(is_dir("../includes/CourseFileManagement/course_files/Projects/".$row['p_id']."/best/")){ $dir = "../includes/CourseFileManagement/course_files/Projects/".$row['p_id']."/Best/";$files = scandir($dir, 0);for($i = 2; $i < count($files); $i++){echo " <a href='../includes/CourseFileManagement/course_files/Projects/".$row['p_id']."/Best/".$files[$i]."' target='_blank'>Download</a>";}} else {echo "<button style='background=none;border:none;' onClick=best('Best');>Add</button>";} ?> <br>
								        	<b>Worst Project :</b> <?php if(is_dir("../includes/CourseFileManagement/course_files/Projects/".$row['p_id']."/worst/")){$dir = "../includes/CourseFileManagement/course_files/Projects/".$row['p_id']."/Worst/";$files = scandir($dir, 0);for($i = 2; $i < count($files); $i++){echo " <a href='../includes/CourseFileManagement/course_files/Projects/".$row['p_id']."/Worst/".$files[$i]."' target='_blank'>Download</a>";}}  else {echo "<button style='background=none;border:none;' onClick=best('Worst');>Add</button>";}  ?> <br>
								        	<br>
						       			<?php
						       			$total = $row['p_total'];
						       			}
							...
							?>
							<center>
								<h2 style='margin-top:5pt;margin-bttom:5pt'>No Project found in System!</h2><br>
								<input type='button' value='Add Project Result' onClick='adMidterm();' class='myButton'   style='margin-left:10pt;'>
							</center>
							<?php }
							if(isset($_GET['add']))
							{?>
		...
		<div class='footer_container'>
		 ...
		 <div id="popup" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;width: 426pt;">
						...
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Add New Project</div>
					   	<form action='../includes/CourseFileManagement/AddProject.php' method='post' enctypt="multipart/from-data">
		 ...
		 <div id="popupa" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						<div class='pop_header'></div>
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Add Marks Project</div>
					   	<form action='../includes/CourseFileManagement/AddProjectMarkssend.php' method='post'>
		 ...
		 <div id="popupb" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						...
						<div class='form-style-2' style='margin-top:-18pt;'>
					   	...
					   	<form action='../includes/CourseFileManagement/AddProjectMarkssend.php' method='post'>
					   		<input type='text' id='p_id_update' name='p_id' hidden> 
