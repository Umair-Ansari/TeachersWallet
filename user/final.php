  ...
  $(function() {
    ...
    $( "#anim" ).change(function() {
      $( "#datepicker" ).datepicker( "option", "showAnim", $( this ).val() );
		...
		function add(user,fname,lname,reg_no,f_id){
				...
				<div class='middle_right'>
					<div class='middle' style='width:100%'>
						<div class='title_middle' style='margin-bottom:0pt;'><h2>Final Term</h2><?php echo $message ?></div>
						       		...
						       		if($row_course = mysqli_fetch_assoc($result_course))
						       		{
							        	...
							        	$result_class = $database->query($Query_class);
							       		...
							       		{ ?>
								        	...
								        	<b>Best Solution :</b> <?php if(is_dir("../includes/CourseFileManagement/course_files/Finalterm/".$row['f_id']."/best/")){echo "<a href='finaltermFiles.php?course=$c_id&type=Best&action=View&term=".$row['f_id']."'>View</a>";} else {echo "<button style='background=none;border:none;' onClick=best('Best');>Add</button>";}  ?> <br>
								        	<b>Worst Solution :</b> <?php if(is_dir("../includes/CourseFileManagement/course_files/Finalterm/".$row['f_id']."/worst/")){echo "<a href='finaltermFiles.php?course=$c_id&type=Worst&action=View&term=".$row['f_id']."'>View</a>";} else {echo "<button style='background=none;border:none;' onClick=best('Worst');>Add</button>";}  ?> <br>
								        	<br>
						       			<?php
						       			$total = $row['f_total'];
						       			}
							...
							?>
							<center>
								<h2 style='margin-top:5pt;margin-bttom:5pt'>No Final Term Result Found in System!</h2><br>
								<input type='button' value='Add Fianl Term Result' onClick='adMidterm();' class='myButton'   style='margin-left:10pt;'>
							</center>
							<?php }
							if(isset($_GET['add']))
							{?>
		...
		<div class='footer_container'>
		 ...
		 <div id="popup" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						...
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Add New Final Term Result</div>
					   	<form action='../includes/CourseFileManagement/AddFinalterm.php' method='post' enctypt="multipart/from-data">
					   		...
					   		<tr>
					   			...
					   			<td conspan='3'  style='padding:0pt 0pt 0pt 5pt;margin-left: -12pt;overflow:hidden'><input type='file' name='Result_mid' accept=".pdf" /> </lable></td>
					   		</tr> -->
					   		<tr>	
		 ...
		 <div id="popupa" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						<div class='pop_header'></div>
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Add Marks Fianl Term Result</div>
					   	<form action='../includes/CourseFileManagement/AddFinaltermMarkssend.php' method='post'>
					   		<input type='text' id='f_id' name='f_id' hidden> 
					   		...
					   		<tr>
					   			...
					   			<td><b>Marks</b></td>
					   			<td>
					   				<input type='text' name='marks' required class="input-field" pattern='[0-9]{1,}' oninvalid="setCustomValidity('must contain number only')" onchange="try{setCustomValidity('')}catch(e){}" >
					   			</td>
					   		<tr>
		 ...
		 <div id="popupb" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						<div class='pop_header'></div>
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Add Marks Final term Result</div>
					   	<form action='../includes/CourseFileManagement/AddFinaltermMarkssend.php' method='post'>
					   		<input type='text' id='m_id_update' name='f_id' hidden> 
			...
			<div id="best" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						...
						<div class='form-style-2' style='margin-top:-18pt;'>
					   	...
					   	<form action='finaltermFiles.php' method='get'>
					   		...
					   		<tr>
					   			...
					   			<input hidden type='text' name='term' value='<?php echo $row['f_id'] ?>' />
