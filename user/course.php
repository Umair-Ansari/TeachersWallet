			...
			<div class='panel'>
				...
				<div class='middle_right'>
					<div class='middle' style='width:100%'>
						<div class='title_middle' style='margin-bottom: 2pt'><h2>Course Request <span title='request from students' class='blink_me'>!</span></h2> <?php echo $message ?></div>
						<div style='width:100%;border:2px solid #E1E2E3;height:30pt; margin-bottom:2pt;'>
							<div class='form-style-2' style='margin-top:-18pt;'>
								
									<input type='button' value='Delete All' class='myButton' onClick="DeleteAll();" style='margin: 1pt 3pt 0pt -13pt;'>
						...
						<div style='width:100%;border:2px solid #E1E2E3;height:426pt;padding:6pt -5pt 0pt 0pt;'>
							<table style='width:100%'>
								...
								$print = "";
								$user = $_SESSION["user"];
								$Query_course_file = "SELECT * FROM course_file WHERE u_id = $user ";
					       		$result_course_file = $database->query($Query_course_file);
								while ( $row_course_file = mysqli_fetch_assoc($result_course_file)) 
								{
									
									$Query_student_request = "SELECT * FROM student_request WHERE c_code = ".$row_course_file['c_code']."";
									...
									{
										$print .= "<tr>";
										$Query_user = "SELECT * FROM user WHERE u_id = ".$row_student_request['u_id']."";
										...
										$Query_student  = "SELECT * FROM student WHERE u_id = ".$row_student_request['u_id']."";
										...
										if($row_course = mysqli_fetch_assoc($result_course ))
										{
											...
											<td style='border:1px solid black;padding:5pt;'><?php echo $reg_no ?></td>
											...
											<td style='border:1px solid black;padding:5pt;'><center><button style='background:none;border:none;border-right:1px solid gray;' title='Accept' onClick="accept('<?php echo $reg_no ?>','<?php echo$name ?>','<?php echo$email ?>','<?php echo$program ?>','<?php echo$wants ?>','<?php echo $row_student_request['sr_id'] ?>');" ><span class='accept'></span></button><button title='Reject' onClick="Delete(<?php echo $row_student_request['sr_id']?>);" style='padding-left: 2pt;background:none;border:none'><span class='reject'></span></button></center></td>
										</tr>
									<?php	
									}
		...
		<div class='footer_container'>
			<div class='footer'><center style='color:white'>2015 Teacher's Wallet</center>
					...
					<div id="popupb" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						<div class='pop_header'></div>
						<div class='form-style-2' style='margin-top:-18pt;'>
					   <form action='../includes/CourseManagement/RejectStudentRequest.php' method='post'>
					   		<table>
