			...
			<div class='panel'>
				...
				<div class='middle_right'>
					<div class='middle' style='width:100%'>
						<div class='title_middle' style='margin-bottom:0pt;'><h2>Assignment<input type='button' value='Add Assignment' class='myButton'  onClick='adMidterm();' style='margin-left:10pt;'></h2><?php echo $message ?></div>
							...
							$Query = "SELECT *  FROM assignment WHERE cf_id =".$c_id."";
						       		...
						       		if($row_course = mysqli_fetch_assoc($result_course))
						       		{
							        	...
							        	$result_class = $database->query($Query_class);
							       		...
							       		{ ?>
							       			...
							       			<center><h3><u>Assignment Result</u></h3></center>
					        ...
					        <table style='width:100%;margin-top:2pt;'>
								...
								<tr style="background-color:rgb(212, 206, 206)">
									...
									<?php
									$Query1 = "SELECT *  FROM assignment WHERE cf_id =".$c_id."";
							        $result1 = $database->query($Query1);
							        while($row1 = mysqli_fetch_assoc($result1))
							        { ?>

							    		<td style='border:1px solid black;padding:5pt;' id='test' contenteditable="true">
											<button style='background:none;border:none;' onClick='assignment_view(<?php echo $row1['assign_id'] ?>);' title='Assignment total marks : <?php echo $row1['assign_total']; ?>'>Assign</button>
											<!-- <a href="#" title='Assignment total marks : <?php echo $row1['assign_total']; ?>'>Assign</a> -->
										</td>
							    	<?php
							        }
							    	?>
							...
							<?php
							    ...
							    ?>
										...
										<?php
										$Query3 = "SELECT *  FROM assignment WHERE cf_id =".$c_id."";
										$result3 = $database->query($Query3);
										while($row3 = mysqli_fetch_assoc($result3))
										{ 
											
											
											$total = $total + $row3['assign_total'];
											$Query4 = "SELECT to_id FROM total WHERE u_id =".$row2['u_id']."";
											$result4 = $database->query($Query4);
											if($row4 = mysqli_fetch_assoc($result4))
											{
												$Query5 = "SELECT * FROM assignment_student WHERE assign_id =".$row3['assign_id']." AND to_id =".$row4['to_id']." ";
												$result5 = $database->query($Query5);
												if($row5 = mysqli_fetch_assoc($result5))
												{
												?> 
													<td><button style='background:none;border:none;cursor:pointer;' onClick="update('<?php echo $row2['fname'] ?>','<?php echo $row2['lname'] ?>','<?php echo $reg_no ?>','<?php echo $row3['assign_total'] ?>','<?php echo $row5['marks'] ?>','<?php echo $row5['as_id'] ?>','<?php echo $row4['to_id'] ?>');" ><?php echo $row5['marks']?></button></td>
											...
											<?php
												...
												{ ?>
													<td><button style='background:none;border:none;cursor:pointer;' onClick="add('<?php echo $row4['to_id'] ?>','<?php echo $row2['fname'] ?>','<?php echo $row2['lname'] ?>','<?php echo $reg_no ?>','<?php echo $row3['assign_id'] ?>','<?php echo $row3['assign_total'] ?>');" >---</button></td>
							...
							?>
							<center>
								<h2 style='margin-top:5pt;margin-bttom:5pt'>No Assignment found in System!</h2><br>
								<input type='button' value='Add Assignment' onClick='adMidterm();' class='myButton'   style='margin-left:10pt;'>
							</center>
							<?php
		...
		<div class='footer_container'>
		 ...
		 <div id="popup" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;width: 426pt;">
						...
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Add New Assignment</div>
					   	<form action='../includes/CourseFileManagement/AddAssignment.php' method='post' enctypt="multipart/from-data">
					   		
					   		<input type='text' value='<?php echo $c_id ?>' name='c_id' hidden> 
					   		<table>
					   		<tr>
					   			<td><label for="field1" style='margin-left: -12pt;'><span style='width: 90pt;'>Assignment Title<span class="required">*</span></span></td>
					   			<td style='padding:0pt 5pt 0pt 5pt' colspan='3'><input type='text' name='title'  class="input-field" style='width:100%;' required ></lable></td>
					   		</tr>  		
					   		<tr>
					   			<td><label for="field1" style='margin-left: -12pt;'><span style='width: 90pt;'>Submission Date <span class="required">*</span></span></td>
		 ...
		 <div id="popupa" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						<div class='pop_header'></div>
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Add Marks Assignment</div>
					   	<form action='../includes/CourseFileManagement/AddAssignmentMarks.php' method='post'>
					   		<input type='text' id='to_id' name='to_id' hidden> 
					   		<input type='text' id='q_id' name='assign_id' hidden> 
					   		<input type='text' value='<?php echo $c_id ?>' name='c_id' hidden> 
					   		<table>
					   		<tr>
					   			<td><b>Reg #</b></td>
		 ...
		 <div id="popupb" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						<div class='pop_header'></div>
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Update Assignment Marks</div>
					   	<form action='../includes/CourseFileManagement/UpdateAssignmentMarks.php' method='post'>
					   		<input type='text' id='qs_id' name='as_id' hidden> 
			...
			<div id="assignment_view" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						<div class='pop_header'></div>
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">View Assignment Files</div>
					   	<form action='ViewAssignment.php' method='get' enctype='multipart/form-data'>
					   		<center>
					   		<table>
					   		<tr>
					   			<td><center><b>Best Case</b></center></td>
					   			<input hidden type='text' name='course' value='<?php echo $c_id ?>' />
					   			<input hidden type='text' name='assignment' id='assignment_id' />
					   	...
					   	<form action='ViewAssignment.php' method='get' enctype='multipart/form-data'>
					   		...
					   		<tr>
					   			...
					   			<input hidden type='text' name='course' value='<?php echo $c_id ?>' />
					   			<input hidden type='text' name='assignment' id='assignment' />
