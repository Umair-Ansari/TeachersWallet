			...
			<div class='panel'>
				...
				<div class='middle_right'>
					<div class='middle' style='width:100%'>
						<div class='title_middle' style='margin-bottom: 2pt'><h2><?php if ($_GET['action'] ==  "View"){ echo "Quiz ".$_GET['case']." Case "; } else {echo "<a href='ViewQuiz.php?course=".$_GET['course']."&action=View&quiz=".$_GET['quiz']."&case=".$_GET['case']."' >Quiz</a>>Upload Quiz Images";}?></h2> <?php echo $message ?></div>
						
						<?php 
						if ($_GET['action'] ==  "View") 
						{
							if(is_dir("../includes/CourseFileManagement/course_files/Quiz/".$_GET['course']."/".$_GET['quiz']."/".$_GET['case']."/"))
        					{
								$dir = "../includes/CourseFileManagement/course_files/Quiz/".$_GET['course']."/".$_GET['quiz']."/".$_GET['case']."/";
								$files = scandir($dir, 0);
							...
							{
								echo "<center><h2 style='margin-top:5pt;margin-bttom:5pt'>No Quiz ".$_GET['case']." Case Files Found in System!</h2><br><button style='margin-left:2pt;'class='myButton' onClick=best();>Add</button></center><br>";

							}
						} else {
							
						?>
						<form action='../includes/CourseFileManagement/AddQuizsFiles.php' method='post' enctype='multipart/form-data'>
							
							<input hidden type='text' name='course' value='<?php echo $_GET['course'] ?>'>
							<input hidden type='text' name='case' value='<?php echo $_GET['case'] ?>' />
					   		<input hidden type='text' name='quiz' value='<?php echo $_GET['quiz'] ?>' /> 
							...
							<col width='100'>
								...
								{ ?>
									...
									<tr>
										...
										<td style='padding-left:2pt;'><input type='file' required accept="image/*" name='<?php echo $i; ?>'></td>
									</tr>
								<?php
								}
								?>
			...
			<div class='footer'><center style='color:white'>2015 Teacher's Wallet</center>
					...
					<div id="best" class='a' style="display:none; background:#F6F7F8; float:left;padding:15pt 15pt 0pt 15pt;">
						...
						<div class='form-style-2' style='margin-top:-18pt;'>
							<div class="form-style-2-heading">Adding Quiz <?php echo $_GET['case'] ?> Case Files</div>
