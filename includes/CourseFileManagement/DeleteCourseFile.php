    ...
    {	
        ...
        if($row_user = mysqli_fetch_assoc($result_user))
        {
            if($row_user['password'] == $this->password)
            {
                        ...
                        $query_attendance = "DELETE FROM  attendance WHERE cf_id =$this->course ";
                        $result_attendance = $database->query($query_attendance);
                        
                        $query_course_student = "DELETE FROM course_student WHERE c_id =$this->course ";
                        $result_course_student = $database->query($query_course_student);
                        
                        $query_midterm = "SELECT m_id FROM midterm WHERE cf_id =$this->course ";
                        ...
                        $query_final = "SELECT f_id FROM final WHERE cf_id =$this->course ";
                        ...
                        $query_project = "SELECT p_id FROM project WHERE cf_id =$this->course ";
                        ...
                        $query_assignment = "SELECT assign_id FROM assignment WHERE cf_id =$this->course ";
                        ...
                        $query_quizzes = "SELECT q_id FROM quizzes WHERE cf_id =$this->course ";
                        ...
                        $query_course_file = "DELETE FROM  course_file WHERE cf_id =$this->course";
                        $result_course_file = $database->query($query_course_file);
                        if ($result_course_file) 
                        {
                            $_SESSION["Message"] = "<div style='background-color:#BCF6BC;border:1px solid green;padding:7pt;'>Course File Deleted</div>";
                            header("Location: ../../user/newCourseFile.php");
                        }
                        else 
                        {
